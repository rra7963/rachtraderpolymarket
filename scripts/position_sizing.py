...
"""Run Phase B Step 5's per-candidate position-sizing/risk_check math over
an already priority-sorted candidate list, per risk_rules.json/
PHASE_B_TASK.md.

Sizing is denominated in USD, which is why this script needed almost no
change from the equity original -- an outcome token bought for $25 is
$25 of exposure the same way a stock is. Share counts come out of
pm_order.py, which walks the real book instead of dividing by a
top-of-book price.

Candidates are processed in the order given (the caller must have
already applied the conviction/risk_flags/liquidity_usd sort) so
...
Candidate JSON is read from stdin, an array of objects:
  {"token_id": "1032...", "slug": "atp-fonseca-oconnel-2026-08-18", "group": "new", "conviction": "high"}
  {"token_id": "9403...", "slug": "atp-nakashi-medvede-2026-08-18", "group": "held", "conviction": "high", "current_position_value": 100.39}
...
def main():
    ...
    p.add_argument("--min-order-usd", type=float, default=1.00,
                    help="risk_rules.json position_sizing.min_order_usd -- "
                         "Polymarket's own per-order minimum")
    ...
    for c in candidates:
        ...
        if args.entries_halted:
            halted_result = {
                "token_id": token_id, "group": group, "conviction": conviction,
                "passed": False,
        ...
        if group == "new":
            ...
            min_top_up_threshold = round(max(args.min_order_usd, args.min_top_up_usd,
            ...
            if top_up_amount < min_top_up_threshold:
                results.append({
                    ...
                    "reason": (
                        ...
                        f"${min_top_up_threshold:.2f} (Polymarket ${args.min_order_usd:.2f} minimum vs. min_top_up_usd "
            ...
            print(json.dumps({"error": f"unknown group '{group}' for token_id {token_id}"}), file=sys.stderr)
            sys.exit(1)
